...
from ..alerts import create_alert_sender, format_stage_alert
...
def monitor_run_once(config: AppConfig, page: PageConfig) -> MonitorResult:
    ...
    sender = create_alert_sender(config.alerts)
    ...
    for raw_conversation in conversations_payload.get("conversations", []):
        ...
        if not config.runtime.dry_run:
            sender.send_alert(format_stage_alert(page, conversation, decision))
            alerted_count += 1
...
def monitor_run_conversation_once(
    config: AppConfig,
    page: PageConfig,
    raw_conversation: dict,
    *,
    staff_user_ids: set[str] | None = None,
) -> MonitorResult:
    """Reconcile a single conversation, intended for webhook-triggered updates."""
    v1 = PageApiClientV1(page.page_access_token)
    store = SQLiteStateStore(config.runtime.state_path)
    sender = create_alert_sender(config.alerts)
...
def monitor_run_loop(config: AppConfig, page: PageConfig, max_iterations: int | None = None) -> None:
    iterations = 0
    while True:
        monitor_run_once(config, page)
        iterations += 1
